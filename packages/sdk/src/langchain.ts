/**
 * LangChain.js integration for FreeChipStore agents.
 * Wraps LangChain with FAGS proxy for API key injection.
 *
 * Usage:
 *   import { createFagsChat, createFagsEmbeddings } from '@freechipstore/sdk/langchain'
...
export function createFagsChat(options: FagsChatOptions) {
  ...
  const origin =
    typeof window !== 'undefined' ? window.location.origin : 'https://freechipstore.online';
 ...
 *   import { getFagsLangChainConfig } from '@freechipstore/sdk/langchain'
...
export function getFagsLangChainConfig(model: string, provider = 'openai') {
  ...
  return {
    ...
    temperature: 0.7,
    configuration: {
      baseURL: `${typeof window !== 'undefined' ? window.location.origin : 'https://freechipstore.online'}/v1/proxy/${host}/v1`,
