...
1. User stores their OpenAI/Anthropic/Google key once at `freechipstore.online/console/` → Profile → API Keys
...
Future: chunking + embeddings via `@freechipstore/smart-search` agent for better retrieval.
...
### Proxy call pattern
```typescript
const response = await fetch('https://freechipstore.online/v1/proxy/api.openai.com/v1/chat/completions', {
  method: 'POST',
  headers: {
...
fags publish      # deploys to freechipstore.online/a/chatbot/
```
