/**
 * FreeChipStore Host Worker
 *
 * Path-based hosting — all agents served from the apex domain:
 *   freechipstore.online/a/{slug}/  → agent app
 *   freechipstore.online/console/   → creator console
 *   freechipstore.online/agents/{slug}/ → detail page
 *   freechipstore.online/            → store site
 *
 * No third-level subdomains for apps. This avoids cookie/PWA/SW isolation
 * issues. MCP stays on mcp.freechipstore.online (separate worker).
 *
 * R2 layout:
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Redirect old subdomain URLs to path-based: tts.freechipstore.online → /a/tts/
    if (
      host !== 'freechipstore.online' &&
      host !== 'www.freechipstore.online' &&
      !host.startsWith('mcp.') &&
      host.endsWith('.freechipstore.online')
    ) {
      const slug = host.split('.')[0];
      return Response.redirect(`https://freechipstore.online/a/${slug}${url.pathname}`, 301);
    }

    // Everything below is on the apex: freechipstore.online
    ...
    if (agentMatch) {
      ...
      const route = await env.DB.prepare('SELECT r2_prefix FROM routes WHERE slug = ? AND zone = ?')
        .bind(slug, 'freechipstore.online')
