...
```
See https://freechipstore.online/skills.md for platform skills.
```
...
FreeChipStore hosts browser-based AI tools in 3 types:
...
All agents are MIT-licensed, open source, one repo per agent in the `FreeChipStore` GitHub org.
...
```bash
npx @freechipstore/cli init my-agent --template heuristic
...
├── agent.json       — manifest
├── package.json     — @freechipstore/my-agent
└── tsconfig.json
```
...
```bash
npx @freechipstore/cli check    # compliance scan
npx @freechipstore/cli publish  # creates repo, deploys, registers route
```
...
- **Header**: FreeChipStore link + agent name + type badge
...
```typescript
fetch('https://freechipstore.online/v1/proxy/api.openai.com/v1/chat/completions', {
  method: 'POST',
...
```bash
npx mcp-remote https://mcp.freechipstore.online/mcp
```
